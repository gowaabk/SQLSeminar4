...
SELECT shopname
FROM shops
WHERE EXISTS (SELECT cats.name FROM cats WHERE shops.id = cats.shops_id AND cats.name = "Murzik");

-- 3. Вывести магазины, в котором НЕ продаются коты “Мурзик” и “Zuza”

SELECT shopname
FROM shops
WHERE NOT EXISTS (SELECT name FROM cats WHERE shops.id = cats.shops_id AND (cats.name = "Murzik" OR cats.name = "Zuza"));
